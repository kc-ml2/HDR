...
def run(config, model):
    ...
    for name in tqdm(namelist):
        SDR = get_test_data(config.test_path, name)
        ...
        # cv2.imwrite(os.path.join(config.save_path,
        #             'hdr_{}.jpg'.format(name)), np.uint8(out*255))
...
if __name__ == "__main__":
    ...
    # Input Parameters
    parser.add_argument('--test_path', type=str, default="valid/")
    ...
    # model_x = MIRNet(32, config.attention_filter,
    #                  config.encoder_kernel, config.triple_pass_filter, config.num_triple_pass, config.filters)
    model_x = Net(config)
